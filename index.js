...
// home
app.route("/").get(routeCb.home).post(routeCb.postHome);

// login
app.route("/login").get(routeCb.login).post(routeCb.postLogin);

// register
app.route("/register").get(routeCb.register).post(routeCb.postRegister);
...
app.get("/auth/google/callback", auth.googleCb, (req, res) =>
  res.redirect("/")
);

// facebook request
app.get("/auth/facebook", auth.facebook);
app.get("/auth/facebook/callback", auth.facebookCb, (req, res) =>
  res.redirect("/")
);

// twitter request
app.get("/auth/twitter", auth.twitter);
app.get("/auth/twitter/callback", auth.twitterCb, (req, res) =>
  res.redirect("/")
);
